export default class Music {
  ...
  _analyerSource(source) {
    this.ac.decodeAudioData(
      ...
      buffer => {
        ...
        this.voiceNode.gain.linearRampToValueAtTime(
          1,
          this.ac.currentTime + Music.FADE_TIME
        );
        // 在曲子快要结束时，淡出之
        this.voiceNode.gain.linearRampToValueAtTime(
          1,
          this.ac.currentTime +
            this.bufferSource.buffer.duration -
            Music.FADE_TIME
        );
        this.voiceNode.gain.linearRampToValueAtTime(
          0,
          this.ac.currentTime + this.bufferSource.buffer.duration
        );
  ...
  _analyserData() {
    ...
    const _round = () => {
      ...
      this._frameTimer = window.requestAnimationFrame(_round.bind(this));
  ...
  _dotedHandler() {
    ...
    for (let i = 0; i < this.size; i++) {
      let x = this._geRande(0, this.canvas.width),
        ...
        color =
          "rgba(" +
          Math.ceil(this._geRande(0, 256)) +
          "," +
          Math.ceil(this._geRande(0, 256)) +
          "," +
          Math.ceil(this._geRande(0, 256)) +
          "," +
          this._geRande(0, 1).toFixed(1) +
          ")";
      dots.push({
        x: x,
        y: y,
